...
void toonz_plugin_exit() { printf("toonz_plugin_exit()\n"); }

void do_compute(toonz::node_handle_t node, const toonz::rendering_setting_t *rs,
                double frame, toonz::tile_handle_t tile) {
  ...
  if (tileif) {
    ...
    for (int i = 0; i < ly; i++) {
      ...
      /* [rx, ry] は effect の面内を描くベクトル. 半径 r 円の中心が [0, 0] で、
         [-lx/2, -ly/2]-[lx/2, ly/2]
...
int do_get_bbox(toonz::node_handle_t node, const toonz::rendering_setting_t *rs,
                double frame, toonz::rect_t *rect) {
  printf("do_get_bbox(): node:%p\n", node);
  bool modified = false;
/* 円の周辺の黒いマスク部分が bbox に相当する:
           コンサバには無限に大きな矩形を返すのが最もコンサバな方法だが、
   rendering_setting_t の affine 変換行列を用いてよりよい予測をすることもできる.
         */
#if 0
	/* too concervative */
...
param_desc_t params0_[] = {
    param_desc_ctor<traits_double_t>(
        "p1st", "", {1.0 /* default */, -1.0 /* min */, 1.0 /* max */}),
    param_desc_ctor<traits_range_t>(
        "p2nd", "", {{48.0, 64.0} /* default */, {0.0, 180.0} /* min-max */}),
    param_desc_ctor<traits_point_t>(
        "p3rd", "",
        {{0, 0} /* default */, {-10, 10} /* min */, {-200, +200} /* max */}),
};

param_desc_t params1_[] = {
    param_desc_ctor<traits_double_t>(
        "g1st", "", {1.0 /* default */, -1.0 /* min */, 1.0 /* max */}),
    param_desc_ctor<traits_range_t>(
        "g2nd", "", {{48.0, 64.0} /* default */, {0.0, 180.0} /* min-max */}),
    param_desc_ctor<traits_point_t>(
        "g3rd", "",
        {{1, 1} /* default */, {-10, 10} /* min */, {-200, +200} /* max */}),
};

param_desc_t params2_[] = {
    param_desc_ctor<traits_double_t>(
        "pp1st", "", {1.0 /* default */, -1.0 /* min */, 1.0 /* max */}),
    param_desc_ctor<traits_range_t>(
        "pp2nd", "", {{48.0, 64.0} /* default */, {0.0, 90.0} /* min-max */}),
    param_desc_ctor<traits_point_t>(
        "pp3rd", "",
        {{1, 1} /* default */, {-10, 10} /* min */, {-200, +200} /* max */}),
};
...
toonz_param_group_t groups0_[] = {
    param_group_ctor("First Group", sizeof(params0_) / sizeof(param_desc_t),
                     params0_),
    param_group_ctor("Second Group", sizeof(params1_) / sizeof(param_desc_t),
                     params1_),
};

toonz_param_group_t groups1_[] = {
    param_group_ctor("Third Group", sizeof(params2_) / sizeof(param_desc_t),
                     params2_),
};

toonz_param_page_t pages_[] = {
    param_page_ctor("Main Page", sizeof(groups0_) / sizeof(toonz_param_group_t),
                    groups0_),
    param_page_ctor("Sub Page", sizeof(groups1_) / sizeof(toonz_param_group_t),
                    groups1_),
};
...
int node_setup(toonz::node_handle_t node) {
  ...
  if (setup) {
    int errcode = 0;
    void *entry = NULL;
    int ret = setup->set_parameter_pages_with_error(node, 2, pages_, &errcode,
                                                    &entry);
    if (ret) {
...
int can_handle(toonz_node_handle_t node, const toonz_rendering_setting_t *rs,
               double frame) {
  printf("%s:\n", __FUNCTION__);
  /* geometric plugin のように Bounding Box が自明の場合、 Skew
     などの変換の影響でフルスクリーン効果のような大きな Bounding Box が
...
TOONZ_PLUGIN_PROBE_DEFINE(TOONZ_PLUGIN_VER(1, 0), "geom-plugin" /* name */,
                          "dwango" /* vendor */, "geom.plugin" /* identifier */,
                          "a geom plugin for test", "http://dwango.co.jp/",
                          TOONZ_PLUGIN_CLASS_POSTPROCESS_SLAB |
                              TOONZ_PLUGIN_CLASS_MODIFIER_GEOMETRIC,
