...
ExrReader::ExrReader() : m_rgbaBuf(nullptr), m_row(0), m_exr_header(nullptr), m_colorSpaceGamma(2.2f) {}
...
void ExrWriter::writeLine(float* buffer) {
  ...
  if (m_bpp == 128) a_p = &m_imageBuf[3][m_row * m_info.m_lx];
  while (pix < endPix) {
    // raster is already linearized (see  MovieRenderer::Imp::postProcessImage() in movierenderer.cpp)
    *r_p++ = pix->r;
    *g_p++ = pix->g;
    *b_p++ = pix->b;
