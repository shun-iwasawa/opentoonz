...
QFrame *OutputSettingsPopup::createPanel(bool isPreview) {
  ...
  if(isPreview)
    m_syncColorSettingsButton = new DVGui::CheckBox(tr("Sync with Output Settings"));
...
QFrame* OutputSettingsPopup::createColorSettingsBox(bool isPreview) {
  ...
  QString colorSpaceGammaTooltip = tr("Color Space Gamma value is used for conversion between the linear and nonlinear color spaces,\n"
    "when the \"Linear Color Space\" option is enabled.");
  if (m_isPreviewSettings)
    colorSpaceGammaTooltip += tr("\nInput less than 1.0 to sync the value with the output settings.");
  ...
  gridLay->setVerticalSpacing(10);
  {
    ...
    gridLay->addWidget(new QLabel(tr("Linear Color Space:"), this), 1, 0,
      Qt::AlignRight | Qt::AlignVCenter);
    gridLay->addWidget(m_linearColorSpaceChk, 1, 1, Qt::AlignLeft | Qt::AlignVCenter);
...
void OutputSettingsPopup::updateField() {
  ...
  if (!prop) {
    ...
    if (m_syncColorSettingsButton) m_syncColorSettingsButton->setCheckState(Qt::Unchecked);
    return;
  }
  ...
  m_channelWidthOm->setDisabled(renderSettings.m_linearColorSpace && renderSettings.m_bpp == 128);

  m_colorSpaceGammaFld->setValue(renderSettings.m_colorSpaceGamma);

  if (m_isPreviewSettings && m_syncColorSettingsButton) {
    m_syncColorSettingsButton->setChecked(getProperties()->isColorSettingsSynced());
  }    
...
void OutputSettingsPopup::onSyncColorSettingsChecked(int state) {
  ...
  if (doSync) {
    TRenderSettings out_rs = getCurrentScene()->getProperties()->getOutputProperties()->getRenderSettings();
    TRenderSettings rs = getProperties()->getRenderSettings();
...
void OutputSettingsPopup::onChannelWidthChanged(int type) {
  ...
  if (!m_isPreviewSettings && getCurrentScene()->getProperties()->getPreviewProperties()->isColorSettingsSynced()) {
    TRenderSettings prev_rs = getCurrentScene()->getProperties()->getPreviewProperties()->getRenderSettings();
    prev_rs.m_bpp = rs.m_bpp;
    getCurrentScene()->getProperties()->getPreviewProperties()->setRenderSettings(prev_rs);
  }
...
void OutputSettingsPopup::onLinearColorSpaceChecked(int state) {
  ...
  // sync output settings value to the preview settings
  TOutputProperties* prev_prop = getCurrentScene()->getProperties()->getPreviewProperties();
...
void OutputSettingsPopup::onPresetSelected(const QString &str) {
  ...
  // in order to make old presets properly reproduce the settings before the implementation of linear rendering
