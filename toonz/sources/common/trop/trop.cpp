...
public:
  ...
  Linear_Lut(int insteps, int outsteps, double gamma) {
    double inspace = (double)(insteps);
    for (int i = 0; i <= insteps; i++) {
      m_table.push_back((Q)((outsteps)*toLinear((double)i / inspace, gamma) + 0.5));
    }
  }
...
void doLinearRGB<TPixelF, float>(TRasterFP raster, double gamma) {
  Linear_Lut<float> lut(TPixel64::maxChannelValue, gamma);  // compute in 0.0-1.0
...
public:
  ...
  sRGB_Lut(int insteps, int outsteps, double gamma) {
    double inspace = (double)(insteps);
    for (int i = 0; i <= insteps; i++) {
      m_table.push_back((Q)((outsteps)*to_sRGB((double)i / inspace, gamma) + 0.5));
    }
  }
...
void TRop::toLinearRGB(TRasterP raster, double gamma, bool sourceIsPremultiplied) {
