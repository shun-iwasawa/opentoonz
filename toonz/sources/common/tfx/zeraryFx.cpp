...
public:
  ...
  void doCompute(TTile &tile, double frame, const TRenderSettings &ri) override {
    ...
    if (!tile.getRaster()->isLinear()) {
      ...
      if (ras32)
        ras32->fill(toLinear(m_color->getPremultipliedValue(frame), ri.m_colorSpaceGamma));
      else if (ras64)
        ras64->fill(toLinear(toPixel64(m_color->getPremultipliedValue(frame)), ri.m_colorSpaceGamma));
      else if (rasF)
        rasF->fill(toLinear(toPixelF(m_color->getPremultipliedValue(frame)), ri.m_colorSpaceGamma));
      else
        throw TException("ColorCardFx unsupported pixel type");
    }
  }
...
public:
  ...
  void doCompute(TTile &tile, double frame,
    ...
    // currently the tile should always be nonlinear
    assert(!isLinear);
    const TPixel32 &c1 = m_color1->getValue(frame, isLinear, info.m_colorSpaceGamma);
    const TPixel32 &c2 = m_color2->getValue(frame, isLinear, info.m_colorSpaceGamma);
