...
ImageLoader::ImageLoader(const TFilePath &path, const TFrameId &fid)
    : m_path(path), m_fid(fid), m_subsampling(0), m_64bitCompatible(false), m_colorSpaceGamma(LevelOptions::DefaultColorSpaceGamma) {}
...
TImageP ImageLoader::build(int imFlags, void *extData) {
  ...
  try {
    ...
    if (m_path.getType() == "exr") {
      // gamma value to be used for converting linear-based image file to nonlinear raster. 
...
bool ImageLoader::isImageCompatible(int imFlags, void *extData) {
  ...
  if (m_path.getType() == "exr" && !areAlmostEqual(m_colorSpaceGamma, sl->getProperties()->colorSpaceGamma()))
    return false;
