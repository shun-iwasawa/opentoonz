...
public:
  ...
  // writeInLinearColorSpace : Whether the format will save image in linear color space. (true only in EXR fromat)
  // writingGamma : Color space gamma to be used for saving in the file ("Color Space Gamma" property in EXR format)
  // renderingGamma : Color space gamma used on rendering ( "Color Space Gamma" value in the Render Settings )
  void postProcessImage(const TRasterImageP &img, bool has64bitOutputSupport,
                        bool writeInLinearColorSpace, bool isFirstTime, double writingGamma,
                        double renderingGamma, const TRasterP &mark, int frame);
      ...
      if (TImageWriterP writerA =
       ...
       if (writeInLinearColorSpace) {
         TDoubleProperty* gammaProp = 
           (TDoubleProperty*)(m_levelUpdaterA->getLevelWriter()->getProperties()->getProperty("Color Space Gamma"));
         if (gammaProp)
           writingGamma = gammaProp->getValue();
       }
      }
...
---*/
    ...
    try {
      ...
      postProcessImage(imgA, has64bitOutputSupport, writeInLinearColorSpace, 
                        ...
                        m_renderSettings.m_colorSpaceGamma, m_renderSettings.m_mark,
                       fid.getNumber());
