...
inline double getVPos(const QPointF & zyPos, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
...
inline float4 getSourcePix(QPointF p, float4* source_host, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
...
inline float getMapValue(QPointF p, float* map_host, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
...
// plane
inline double getXPos(QVector3D pos, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
  double angle_Q = vars.angle_el - atan((vars.P_y - pos.y()) / pos.z());
  double dist = vars.d_PT * cos(vars.angle_el - angle_Q) / (cos(angle_Q) * pos.z());
  return pos.x() * dist + double(vars.resultDim.lx) * 0.5;
};

inline float4 getTextureOffsetColor(double preTexOff, double texOff, float ratio) {
  float val = lerp(preTexOff, texOff, ratio);
  return float4{ val, val, val, 1.0 };
};
inline float4 getDiffuseColor(QVector3D n, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
  float val = QVector3D::dotProduct(n, vars.sunVec);
  if (val < 0.0) val = 0.0;
  return float4{ val, val, val, 1.0 };
};
inline float4 getDiffuseDifferenceColor(QVector3D n, QVector3D base_n, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
  float val = QVector3D::dotProduct(n, vars.sunVec);
...
inline float4 getSpecularColor(QVector3D n, QVector3D pos, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
...
inline float4 getSpecularDifferenceColor(QVector3D n, QVector3D base_n, QVector3D pos, QVector3D base_pos, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
  QVector3D halfVector =
    ((vars.eyePos - pos).normalized() + vars.sunVec).normalized();
...
inline float4 getFresnelColor(QVector3D n, QVector3D pos, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
  float angle = acos(QVector3D::dotProduct(n, (vars.eyePos - pos).normalized()));
  float val =
    (fresnelFactor(angle) - vars.base_fresnel_ref) / (1.0f - vars.base_fresnel_ref);
  if (val < 0.0) val = 0.0;
  return float4{ val, val, val, 1.0 };
};
inline float4 getFresnelDifferenceColor(QVector3D n, QVector3D base_n, QVector3D pos, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
  float val = getFresnelColor(n, pos, vars).x - getFresnelColor(base_n, pos, vars).x;
  if (val < 0.0) val = 0.0;
  return float4{ val, val, val, 1.0 };
};
...
inline float4 getRefractionColor(QVector3D n, QVector3D pos, float4* source_host, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
  ...
  QVector3D refrRay = sin(angle_refr) * (n_eye * n - eyeVec).normalized() -
    cos(angle_refr) * n;
  double travelLength = -(vars.depth + pos.y()) / refrRay.y();
  QVector3D bottomPos = pos + refrRay * travelLength;
  return getSourcePix(QPointF(
    getXPos(bottomPos, vars), getVPos(QPointF(bottomPos.z(), bottomPos.y()), vars)), source_host, vars);
};

inline float4 getReflectionColor(QVector3D n, QVector3D pos, float4* source_host, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
  ...
  if (vars.distance < 0.0) {
    ...
    double ref_v = vars.d_PT * tan(vars.angle_el - angle_ref_elev) + vars.H / 2.0;
    double ref_u =
  ...
  return getSourcePix(QPointF(
    getXPos(boardPos, vars), getVPos(QPointF(boardPos.z(), boardPos.y()), vars)), source_host, vars);
};

inline float4 getColor(QVector3D pre_n, QVector3D cur_n, QVector3D pre_p,
  QVector3D cur_p, float ratio, float4* source_host, const Iwa_FloorBumpFx::FloorBumpVars & vars,
  QVector3D pre_base_n = QVector3D(0,1,0), QVector3D cur_base_n = QVector3D(0, 1, 0),
  QVector3D pre_base_pos = QVector3D(), QVector3D cur_base_pos = QVector3D()) {
  if (vars.renderMode == Iwa_FloorBumpFx::DiffuseMode) {
    if (vars.differenceMode)
      return getDiffuseDifferenceColor(lerpNormal(pre_n, cur_n, ratio), lerpNormal(pre_base_n, cur_base_n, ratio), vars);
    else
      return getDiffuseColor(lerpNormal(pre_n, cur_n, ratio), vars);
  }
  else if (vars.renderMode == Iwa_FloorBumpFx::SpecularMode) {
    if (vars.differenceMode)
      return getSpecularDifferenceColor(lerpNormal(pre_n, cur_n, ratio),
        lerpNormal(pre_base_n, cur_base_n, ratio), lerpPos(pre_p, cur_p, ratio), 
    ...
    if (vars.differenceMode)
      return getFresnelDifferenceColor(lerpNormal(pre_n, cur_n, ratio), lerpNormal(pre_base_n, cur_base_n, ratio), lerpPos(pre_p, cur_p, ratio), vars);
    else
      return getFresnelColor(lerpNormal(pre_n, cur_n, ratio), lerpPos(pre_p, cur_p, ratio), vars);
  }
...
QList<QPointF> getSubPointsList(int subAmount, const Iwa_FloorBumpFx::FloorBumpVars & vars) {
  QList<QPointF> ret;
...
Iwa_FloorBumpFx::Iwa_FloorBumpFx()
  ...
  m_waveHeight->setValueRange(0.0, 1000.0);
...
bool Iwa_FloorBumpFx::doGetBBox(double frame, TRectD &bBox,
  ...
  if (m_texture.isConnected()) {
    bool ret = m_texture->doGetBBox(frame, bBox, info);
...
inline void Iwa_FloorBumpFx::initVars(FloorBumpVars& vars, TTile &tile, const TRenderSettings &settings, double frame) {
  ...
  vars.sourceDim = TDimensionI(tceil(vars.precision * double(vars.resultDim.lx + vars.margin * 2)),
    tceil(vars.precision * double(vars.resultDim.ly + vars.margin * 2)));
  // only add margins for height image
  vars.refDim = TDimensionI(vars.resultDim.lx + vars.margin * 2, vars.refHeight + vars.margin * 2);
  ...
  vars.A = cam_tilt.map(QPointF(vars.d_PT, vars.H / 2.0)) + QPointF(0, vars.P_y);
  // Z-Y position of the center of bottom edge of the projection plane (B)
  vars.B = cam_tilt.map(QPointF(vars.d_PT, -vars.H / 2.0)) + QPointF(0, vars.P_y);
  ...
  case ReflectionMode: {
    ...
    vars.distance = (angle_dl >= vars.angle_el) ? -1 : vars.P_y / tan(vars.angle_el - angle_dl);
    break;
  }
  }
...
void Iwa_FloorBumpFx::doCompute_CPU(
    TTile &tile, const double frame, const TRenderSettings &settings,
    const FloorBumpVars& vars, float4 *source_host, float *ref_host, float4 *result_host) {
  ...
  if (vars.renderMode == TextureMode) {
    ...
    for (int i = 0; i < vars.resultDim.lx; i++) {
      ...
      for (int j = -vars.margin; j < vars.refHeight; j++) {
        ...
        double angle_Q = std::atan((float(j) - (vars.outDim.ly / 2.0)) / vars.d_PT);
        ...
        // (R)
        float refVal = getMapValue(QPointF(double(i), double(j)), ref_host, vars);
        ...
        if (vars.textureOffsetAmount == 0.0)
          ...
          // projection plane
          double dist = (vars.d_PT * sin(vars.angle_el - angle_Q)) / vars.P_y * cos(angle_Q);
          ...
          offset *= vars.textureOffsetAmount / (vars.spread* (0.5 + 0.5/ float(subAmount))); //距離
          currentUV = QPointF(double(i) + offset.x() * dist,
                              getVPos(QPointF(R_z + offset.y(), 0), vars));
          if (vars.differenceMode)
            currentTextureOffset = std::sqrt(offset.x()*offset.x() + offset.y()*offset.y()) / vars.textureOffsetAmount;
        ...
        if (currentY == toY) {
          ...
          if (vars.differenceMode)
            *result_p += getTextureOffsetColor(preTextureOffset, currentTextureOffset, 0.5) * ratio;
          else
            *result_p += getSourcePix(lerpUV(preUV, currentUV, 0.5), source_host, vars) * ratio;
          preVPos = vPos;
          maxVPos = vPos;
          preUV   = currentUV;
          preTextureOffset = currentTextureOffset;
        ...
        if (vars.differenceMode)
          *result_p += getTextureOffsetColor(preTextureOffset, currentTextureOffset, k) * ratio;
        else
          *result_p += getSourcePix(lerpUV(preUV, currentUV, k), source_host, vars) * ratio;
        ...
        for (int y = currentY + 1; y < toY; y++) {
          ...
          if (vars.differenceMode)
            *result_p += getTextureOffsetColor(preTextureOffset, currentTextureOffset, k);
          else
            *result_p = getSourcePix(lerpUV(preUV, currentUV, k), source_host, vars);
          if (result_p == end_p) break;
          result_p++;
        }
        ...
        if (vars.differenceMode)
          *result_p += getTextureOffsetColor(preTextureOffset, currentTextureOffset, k) * to_frac;
        else
          *result_p += getSourcePix(lerpUV(preUV, currentUV, k), source_host, vars) * to_frac;
  ...
  // (Diffuse, Specular, Fresnel, Refraction, Reflection)
    ...
    for (int i = 0; i < vars.resultDim.lx; i++) {
      ...
      QVector3D prePos(i, 0, vars.C_z);  // actually the x coordinate is not i at the
      ...
      for (int j = -vars.margin; j < vars.refHeight; j++) {
        ...
        if (vars.renderMode == ReflectionMode && vars.distance > 0.0 && R_z >= vars.distance) break;
        ...
        // plane
        double dist     = (vars.d_PT * sin(vars.angle_el - angle_Q)) / vars.P_y * cos(angle_Q);
        double xRefGrad = getMapValue(QPointF(double(i) + dist, double(j)), ref_host, vars) -
          getMapValue(QPointF(double(i) - dist, double(j)), ref_host, vars);
        double zRefGrad =
          getMapValue(QPointF(double(i), getVPos(QPointF(R_z + 1, 0), vars)), ref_host, vars) -
          getMapValue(QPointF(double(i), getVPos(QPointF(R_z - 1, 0), vars)), ref_host, vars);
        ...
        // height of (S) projected on the projection plane
        double vPos = getVPos(S, vars);
        QVector3D currentPos(double(i - vars.resultDim.lx / 2) / dist, S.y(), S.x());
        ...
        if (currentY == toY) {
          float ratio = to_frac - current_frac;
          *result_p +=
              getColor(preNormal, currentNormal, prePos, currentPos, 0.5, source_host, vars) *
              ratio;
          preVPos   = vPos;
          maxVPos   = vPos;
          preNormal = currentNormal;
        ...
        float k =
            (maxVPos + (1.0 - current_frac) / 2.0 - preVPos) / (vPos - preVPos);
        float4 color =
            getColor(preNormal, currentNormal, prePos, currentPos, k, source_host, vars);
        float ratio = 1.0 - current_frac;
        *result_p += color * ratio;
        ...
        for (int y = currentY + 1; y < toY; y++) {
          k         = (float(y) + 0.5 - preVPos) / (vPos - preVPos);
          *result_p = getColor(preNormal, currentNormal, prePos, currentPos, k, source_host, vars);
          if (result_p == end_p) break;
          result_p++;
        }
        ...
        // fill fraction part of the end pixel (toY)
        k     = (float(toY) + to_frac * 0.5 - preVPos) / (vPos - preVPos);
        color = getColor(preNormal, currentNormal, prePos, currentPos, k, source_host, vars);
...
void Iwa_FloorBumpFx::doCompute_with_Displacement(
  ...
  float4 *source_host, float *ref_host, float *disp_host, float4 *result_host) {
    ...
    for (int i = 0; i < vars.resultDim.lx; i++) {
      ...
      for (int j = -vars.margin; j < vars.refHeight; j++) {
        ...
        if (vars.textureOffsetAmount == 0.0) {
          ...
          if (vars.differenceMode)
            currentTextureOffset = 1000 * std::sqrt(offset.x()*offset.x() + offset.y()*offset.y()) / vars.textureOffsetAmount;
        }
        QPointF currentDispUV = QPointF(double(i) + uvOffset.x(),
        ...
        // 変位後の表面座標 (M) ※法線のX成分は強引にY座標の変位に持ち込んで近似する
        float dispXY = std::sqrt(currentNormal.x()*currentNormal.x() + currentNormal.y()*currentNormal.y());
        QPointF M = S + QPointF(currentNormal.z(), dispXY) * dispHeight;
        //float dispXRatio = std::sqrt(currentNormal.x()*currentNormal.x() + currentNormal.y()*currentNormal.y()) / currentNormal.y();
        //QPointF M = S + QPointF(currentNormal.z(), currentNormal.y()* dispXRatio) * dispHeight;

        // height of (M) projected on the projection plane
        double vPos = getVPos(M, vars);

        uvOffset.setY(dispHeight);
        QPointF currentUV = QPointF(double(i) + uvOffset.x(),
        ...
        if ((vars.displacement > 0.0 && dispValue < 1.0) ||
          ...
          for (int k = 1; k < 10; k++, tmpUV += dUV, tmpHeight += dHeight) {
            if (getMapValue(tmpUV, disp_host, vars)*vars.displacement > tmpHeight) {
              isBehind = true;
              break;
            }
          }
  ...
  // (Diffuse, Specular, Fresnel, Refraction, Reflection)
    ...
    for (int i = 0; i < vars.resultDim.lx; i++) {
      ...
      for (int j = -vars.margin; j < vars.refHeight; j++) {
        ...
        // この地点の変位の値を取得する
        float dispHeight = getMapValue(currentDispUV, disp_host, vars)*vars.displacement;
        
        // compute normal vector from cross-product of surface vectors
        QVector3D rightPos(1.0, getMapValue(QPointF(double(i) + dist, double(j)), ref_host, vars) * vars.waveHeight, 0.0);
        QVector3D leftPos(-1.0, getMapValue(QPointF(double(i) - dist, double(j)), ref_host, vars) * vars.waveHeight, 0.0);
        QVector3D farPos(0.0,  getMapValue(QPointF(double(i), getVPos(QPointF(R_z + 1, 0), vars)), ref_host, vars) * vars.waveHeight, 1.0);
        QVector3D nearPos(0.0, getMapValue(QPointF(double(i), getVPos(QPointF(R_z - 1, 0), vars)), ref_host, vars) * vars.waveHeight, -1.0);
        // 左手系座標なので外積を反転する
        QVector3D currentNormal = -QVector3D::crossProduct(rightPos - leftPos, farPos - nearPos);
        currentNormal.normalize();
        ...
        QPointF farDispUV(currentDispUV.x(), getVPos(QPointF(R_z + uvOffset.z() + 1, 0), vars));
        QPointF nearDispUV(currentDispUV.x(), getVPos(QPointF(R_z + uvOffset.z() - 1 , 0), vars));
        float rightDispHeight = getMapValue(rightDispUV, disp_host, vars)*vars.displacement;
        ...
        float nearDispHeight = getMapValue(nearDispUV, disp_host, vars)*vars.displacement;
        ...
        QVector3D dispNormal = -QVector3D::crossProduct(rightPos - leftPos, farPos - nearPos);
        dispNormal.normalize();
        
        // height of (M) projected on the projection plane
        double vPos = getVPos(M, vars);
        QVector3D currentPos(double(i - vars.resultDim.lx / 2) / dist, M.y(), M.x());
        ...
        QVector3D basePos;
        if (vars.differenceMode) {
          baseNormal = -QVector3D::crossProduct(QVector3D(2.0, rightDispHeight - leftDispHeight, 0.0),
            QVector3D(0.0, farDispHeight - nearDispHeight, 2.0));
          basePos = QVector3D(currentPos.x() + uvOffset.x() / dist, dispHeight, currentPos.z() + uvOffset.z());
        }
        ...
        if (currentY == toY) {
          float ratio = to_frac - current_frac;
          *result_p +=
            getColor(preDispNormal, dispNormal, prePos, currentPos, 0.5, source_host, vars, preBaseNormal, baseNormal, preBasePos, basePos) *
            ratio;
          preVPos = vPos;
          maxVPos = vPos;
          preDispNormal = dispNormal;
        ...
        float k =
          (maxVPos + (1.0 - current_frac) / 2.0 - preVPos) / (vPos - preVPos);
        float4 color =
          getColor(preDispNormal, dispNormal, prePos, currentPos, k, source_host, vars, preBaseNormal, baseNormal, preBasePos, basePos);
        float ratio = 1.0 - current_frac;
        *result_p += color * ratio;
...
void Iwa_FloorBumpFx::setRefRaster(const TRaster64P refRas, float *refMem,
  ...
  float zeroLevel = (isRef) ? float(128) / float(TPixel32::maxChannelValue) : 0.0;
  float *r_p = refMem;
  for (int j = 0; j < dim.ly; j++) {
...
void Iwa_FloorBumpFx::doCompute(TTile &tile, double frame,
  ...
  if (isTextureUsed(vars.renderMode)) {
    source_host_ras = TRasterGR8P(vars.sourceDim.lx * sizeof(float4), vars.sourceDim.ly);
  ...
  if (vars.displacement != 0.0 && m_dispRef.isConnected()) { // renderMode すべてに対応させるかはTBD
  ...
  // 画像の取得、格納
  {
    if (isTextureUsed(vars.renderMode)) {
      ...
      if (ras32)
        setSourceRaster<TRaster32P, TPixel32>(ras32, source_host, vars.sourceDim);
      else if (ras64)
        setSourceRaster<TRaster64P, TPixel64>(ras64, source_host, vars.sourceDim);
    }
    ...
    ref_sets.m_bpp     = 64;
    TTile refTile;
    m_heightRef->allocateAndCompute(refTile, refTilePos, vars.refDim, TRasterP(),
                                    frame, ref_sets);
    ...
    if (disp_host) {
      TTile dispTile;
      m_dispRef->allocateAndCompute(dispTile, refTilePos, vars.refDim, TRasterP(),
        frame, ref_sets);
      setRefRaster((TRaster64P)dispTile.getRaster(), disp_host, vars.refDim, false);
    }
  }

  // prepare the memory for result image
  TRasterGR8P result_host_ras(vars.resultDim.lx * sizeof(float4), vars.resultDim.ly);
  float4 *result_host;
  ...
  // compute
  if (disp_host) {
    doCompute_with_Displacement(tile, frame, rend_sets, vars, 
      source_host, ref_host, disp_host, result_host);
  }
  else {
