...
void Iwa_MotionBlurCompFx::convertRGBtoExposure_CPU(
    float4 *in_tile_p, TDimensionI &dim, ExposureConverter &conv,
...
void Iwa_MotionBlurCompFx::doCompute_CPU(
  ...
  if (getFxVersion() == 1)
    convertRGBtoExposure_CPU(in_tile_p, enlargedDimIn,
                             HardnessBasedConverter(gamma, settings.m_colorSpaceGamma, enlarge_tile.getRaster()->isLinear()),
  ...
  if (m_background.isConnected()) {
    if (getFxVersion() == 1)
      composeBackgroundExposure_CPU(out_tile_p, enlargedDimIn, marginRight,
                                    ...
                                    HardnessBasedConverter(gamma, settings.m_colorSpaceGamma, tile.getRaster()->isLinear()));
    else
      composeBackgroundExposure_CPU(out_tile_p, enlargedDimIn, marginRight,
                                    marginTop, back_tile, dimOut,
...
bool Iwa_MotionBlurCompFx::toBeComputedInLinearColorSpace(bool settingsIsLinear, bool tileIsLinear) const {
