...
Iwa_BokehAdvancedFx::Iwa_BokehAdvancedFx()
  ...
  // Version 2: Exposure can also be computed by using Gamma, for easier combination with
...
void Iwa_BokehAdvancedFx::onFxVersionSet() {
  ...
  for (int layer = 0; layer < LAYER_NUM; layer++) {
    getParams()->getParamVar(QString("gamma%1").arg(layer + 1).toStdString())->setIsHidden(!useGamma);
    getParams()->getParamVar(QString("gammaAdjust%1").arg(layer + 1).toStdString())->setIsHidden(useGamma);
  }
}
...
void Iwa_BokehAdvancedFx::doCompute(TTile& tile, double frame,
  ...
  for (auto index : sourceIndices) {
    ...
    m_layerParams[index].m_source->allocateAndCompute(
        *layerTile, _rectOut.getP00(), dimOut, tile.getRaster(), frame, settings);
  ...
  if (m_linearizeMode->getValue() == Hardness)
    ...
    if (getFxVersion() == 2)
      ...
      masterGamma = std::max(1., settings.m_colorSpaceGamma + m_gammaAdjust->getValue(frame));
    ...
    if (m_hardnessPerSource->getValue()) {
      if (m_linearizeMode->getValue() == Hardness)
        ...
        if (getFxVersion() == 2)
          ...
          layerValue.layerGamma = std::max(1., settings.m_colorSpaceGamma +
            m_layerParams[index].m_gammaAdjust->getValue(frame));
        if (tile.getRaster()->isLinear()) layerValue.layerGamma /= settings.m_colorSpaceGamma;
      }
    } else
