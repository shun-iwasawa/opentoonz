...
Iwa_AdjustExposureFx::Iwa_AdjustExposureFx()
    : m_hardness(3.3), m_scale(0.0), m_offset(0.0), m_gamma(2.2), m_gammaAdjust(0.) {
...
void Iwa_AdjustExposureFx::doCompute(TTile &tile, double frame,
  ...
  if (getFxVersion() == 1) // hardness
    ...
    if (getFxVersion() == 2)
      ...
      gamma = std::max(1., settings.m_colorSpaceGamma + m_gammaAdjust->getValue(frame));
  ...
  if (TRasterFP rasF = tile.getRaster()) {
    if (getFxVersion() == 1)
      doFloatCompute(rasF, frame, dim, HardnessBasedConverter(gamma, settings.m_colorSpaceGamma, tile.getRaster()->isLinear()));
    else
  ...
  if (getFxVersion() == 1)
    doCompute_CPU(frame, dim, tile_host, HardnessBasedConverter(gamma, settings.m_colorSpaceGamma, tile.getRaster()->isLinear()));
  else
...
bool Iwa_AdjustExposureFx::toBeComputedInLinearColorSpace(bool settingsIsLinear, bool tileIsLinear) const {
