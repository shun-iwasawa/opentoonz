...
template <>
void doRGBKey(TRasterFP ras, TPixelF highColor, TPixelF lowColor,
  bool gender) {
  int j;
  ras->lock();
  for (j = 0; j < ras->getLy(); j++) {
...
void RGBKeyFx::doCompute(TTile &tile, double frame, const TRenderSettings &ri) {
  ...
  if (raster32)
    doRGBKey<TPixel32>(raster32, toPixel32(highColor),
                              toPixel32(lowColor), gender);
  else if (raster64)
    doRGBKey<TPixel64>(raster64, toPixel64(highColor),
                               toPixel64(lowColor), gender);
  else if (rasterF)
