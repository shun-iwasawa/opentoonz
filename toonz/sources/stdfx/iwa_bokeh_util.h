...
public:
  ...
  virtual double valueToExposure(double value)    = 0;
  virtual double exposureToValue(double exposure) = 0;
...
public:
  HardnessBasedConverter(double gamma, double colorSpaceGamma, bool fromLinear = false) 
    : ExposureConverter(gamma), m_fromLinear(fromLinear), m_colorSpaceGamma(colorSpaceGamma) {}
  double valueToExposure(double value) final override {
  ...
  double exposureToValue(double exposure) final override {
    double ret = std::log10(exposure) * m_gamma + 0.5;
    if (m_fromLinear && ret > 0.)
      ret = std::pow(ret, m_colorSpaceGamma);
    return ret;
  }
...
protected:
  ...
  TDoubleParamP m_gammaAdjust;  // Gamma offset from the current color space gamma (Version 3)
