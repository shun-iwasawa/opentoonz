...
template <>
void float_arr_to_ras_<TPixelF>(const float* arr, const int channels, TRasterFP ras,
  const int margin  // default is 0
) {
  arr +=
    (ras->getLx() + margin + margin) * margin * channels + margin * channels;
...
void ino::arr_to_ras(const unsigned char* in_arr, const int channels,
  ...
  if ((TRaster32P)out_ras) {
    ...
    arr_to_ras_<float, TPixelF>(
      reinterpret_cast<const float*>(in_arr), channels, out_ras,
      margin);
  }
}
void ino::float_arr_to_ras(const unsigned char* in_arr, const int channels,
  ...
  if ((TRaster32P)out_ras) {
    ...
    float_arr_to_ras_<TPixelF>(reinterpret_cast<const float*>(in_arr),
                                channels, out_ras, margin);
  }
}
//--------------------
...
void TBlendForeBackRasterFx::onFxVersionSet() {
  ...
  if (useGamma) {
    ...
    if (m_gamma->getKeyframeCount() == 0 && areAlmostEqual(m_gamma->getDefaultValue(), 2.2)) {
      useGamma = false;
      // call onObsoleteParamLoaded here in case loading the old fx before introducing the linear option
...
// loaded
void TBlendForeBackRasterFx::onObsoleteParamLoaded(const std::string& paramName) {
...
void TBlendForeBackRasterFx::doCompute(TTile& tile, double frame,
  ...
  bool linear_sw = toBeComputedInLinearColorSpace(rs.m_linearColorSpace, tile.getRaster()->isLinear());
  ...
  try {
    ...
    doComputeFx(dn_ras, up_ras, TPoint(), up_opacity, gamma / rs.m_colorSpaceGamma, rs.m_colorSpaceGamma, linear_sw);
  ...
  for (int yy = 0; yy < dn_ras_out->getLy(); ++yy) {
    ...
    for (; out_pix < out_end; ++out_pix, ++up_pix) {
      ...
      if (dna > 0.0) { 
        for (int c = 0; c < 3; c++) {
          if (premultiplied_sw)
            dnBGR[c] = to_linear_color_space(dnBGR[c] / dna, 1.0, gammaDif) * dna;
          else
...
//------------------------------------------------------------
template <class T, class Q>
void TBlendForeBackRasterFx::premultiToUnpremulti(TRasterPT<T> dn_ras,
  const TRasterPT<T>& up_ras,
  const double colorSpaceGamma) {
...
void TBlendForeBackRasterFx::premultiToUnpremulti<TPixelF, float>(
  TRasterFP dn_ras, const TRasterFP& up_ras,
  const double colorSpaceGamma) {
  assert(dn_ras->getSize() == up_ras->getSize());
  assert(dn_ras->isLinear() == up_ras->isLinear());
  ...
  for (int yy = 0; yy < dn_ras->getLy(); ++yy) {
    ...
    for (; dn_pix < dn_end; ++dn_pix, ++up_pix) {
      if (up_pix->m > 0.f && up_pix->m < 1.f) {
        float up_fac = std::pow(up_pix->m, static_cast<float>(colorSpaceGamma - 1.));
        up_pix->r *= up_fac;
        up_pix->g *= up_fac;
        up_pix->b *= up_fac;
      }
      if (dn_pix->m > 0.f && dn_pix->m < 1.f) {
        float dn_fac = std::pow(dn_pix->m, static_cast<float>(colorSpaceGamma - 1.));
        dn_pix->r *= dn_fac;
        dn_pix->g *= dn_fac;
        dn_pix->b *= dn_fac;
      }
...
bool TBlendForeBackRasterFx::toBeComputedInLinearColorSpace(bool settingsIsLinear, bool tileIsLinear) const {
  ColorSpaceMode mode = static_cast<ColorSpaceMode>(m_colorSpaceMode->getValue());
