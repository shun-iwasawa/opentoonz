...
------------------------------------*/
void Iwa_SpectrumFx::calcBubbleMap(float3 *bubbleColor, double frame, bool computeAngularAxis) {
  int i, j, k;     /*- bubbleColor[j][k] = [256][3] -*/
  float d;      /*- 膜厚(μm) -*/
  int ram;      /*- 波長のfor文用 -*/
  float rambda; /*- 波長(μm) -*/
  struct REFLECTIVITY {
    float r_ab, t_ab, r_ba, t_ba; /* transmission and reflection amplitudes for each boundary */
  ...
  for ( i = 0; i < i_max; i++){
    ...
    /* transmission and reflection amplitudes for each boundary, for each polarization */
    float cos_in = cosf(angle_in);
    float cos_re = cosf(angle_re);
    //P-polarized light
    p.r_ab = (cos_re - refractiveIndex*cos_in) / (cos_re + refractiveIndex*cos_re);
    p.t_ab = (1.0f - p.r_ab) / refractiveIndex;
    p.r_ba = -p.r_ab;
    p.t_ba = (1.0f + p.r_ab) * refractiveIndex;
    //S-polarized light
    s.r_ab = (cos_in - refractiveIndex*cos_re) / (cos_in + refractiveIndex*cos_re);
    s.t_ab = 1.0f + s.r_ab;
    s.r_ba = -s.r_ab;
    s.t_ba = 1.0f - s.r_ab;
