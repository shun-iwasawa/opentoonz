...
void Iwa_SpectrumFx::calcBubbleMap(float3 *bubbleColor, double frame,
                                   bool isLinear, double colorSpaceGamma, bool computeAngularAxis) {
  ...
  float gammaAdjust = (getFxVersion() == 1 && isLinear)    ? (float)colorSpaceGamma
                      : (getFxVersion() >= 2 && !isLinear) ? 1.f / (float)colorSpaceGamma
                                                           : 1.f;
...
void Iwa_SpectrumFx::doCompute(TTile &tile, double frame,
  ...
  /*- シャボン色マップの生成 -*/
  calcBubbleMap(bubbleColor, frame, tile.getRaster()->isLinear(), settings.m_colorSpaceGamma);
