...

  {
    ...
    for (int i = 0; i < dim.lx * dim.ly; i++, alp_p++, res_p++) {
      ...
      if (m_masterGamma != m_layerGamma) {
        if (isGammaBased())
        exposure = std::pow(exposure / (*alp_p), m_masterGamma / m_layerGamma) *
          (*alp_p);
        else // hardness based
          exposure = std::pow(exposure / (*alp_p), m_layerGamma / m_masterGamma) *
            (*alp_p);
      }
...
void BokehUtils::setOutputRaster<TRasterFP, TPixelF>(double4* src,
  ...
  for (int j = 0; j < dstRas->getLy(); j++) {
    ...
    src_p += margin.x;
    for (int i = 0; i < dstRas->getLx(); i++, outPix++, src_p++) {
      outPix->r = (typename TPixelF::Channel)((std::isfinite((*src_p).x) && (*src_p).x > 0.) ? (*src_p).x : 0.);
      outPix->g = (typename TPixelF::Channel)((std::isfinite((*src_p).y) && (*src_p).y > 0.) ? (*src_p).y : 0.);
      outPix->b = (typename TPixelF::Channel)((std::isfinite((*src_p).z) && (*src_p).z > 0.) ? (*src_p).z : 0.);
...
void Iwa_BokehCommonFx::doFx(TTile& tile, double frame,
  ...
  for (int i = 0; i < layerValues.size(); i++) {
    ...
    if (-1.0 <= irisSize && 1.0 >= irisSize) {
      ...
      if (m_linearizeMode->getValue() == Hardness)
        BokehUtils::compositLayerAsIs(*layerTile, result, dimOut,
                                      HardnessBasedConverter(masterGamma, settings.m_colorSpaceGamma, layerTile->getRaster()->isLinear()));
      else
        BokehUtils::compositLayerAsIs(*layerTile, result, dimOut,
    ...
    if (m_linearizeMode->getValue() == Hardness)
      conv.reset(new HardnessBasedConverter(layerGamma, settings.m_colorSpaceGamma, layerTile->getRaster()->isLinear()));
    else
  ...
  if (m_linearizeMode->getValue() == Hardness)
    BokehUtils::convertExposureToRGB(result, dimOut.lx * dimOut.ly,
                                     HardnessBasedConverter(masterGamma, settings.m_colorSpaceGamma, tile.getRaster()->isLinear()));
  else
    BokehUtils::convertExposureToRGB(result, dimOut.lx * dimOut.ly,
                                     GammaBasedConverter(masterGamma));
...
void Iwa_BokehCommonFx::doBokehRef(
  ...
  if (m_linearizeMode->getValue() == Hardness)
    BokehUtils::convertRGBToExposure(source_buff, size,
                                     HardnessBasedConverter(layerGamma, settings.m_colorSpaceGamma, layer.sourceTile->getRaster()->isLinear()));
  else
    BokehUtils::convertRGBToExposure(source_buff, size,
  ...
  double adjustFactor = (m_linearizeMode->getValue() == Hardness) ? layerGamma / masterGamma : masterGamma / layerGamma;
