...
void change_(float* image_array, const int height, const int width,
  ...
  const unsigned int table_size = std::numeric_limits<unsigned short>::max() + 1;
  ...
  //const double mul_val = div_val + 0.999999;
  {
    ...
    for (unsigned int yy = 0; yy < table_size; ++yy) {
      double rr, gg, bb, aa;
      rr = gg = bb = aa = yy / div_val;
      levels_(rr, r_in_min, r_in_max, r_gamma, r_out_min, r_out_max, false);  // クランプしない
      levels_(gg, g_in_min, g_in_max, g_gamma, g_out_min, g_out_max, false);  // クランプしない
      levels_(bb, b_in_min, b_in_max, b_gamma, b_out_min, b_out_max, false);  // クランプしない
      levels_(aa, a_in_min, a_in_max, a_gamma, a_out_min, a_out_max, clamp_sw);
      /* 0〜1.0 --> 0〜mul_maxスケール変換し、整数値化 */
  ...
  auto getTableVal = [&](int channel, float in) {
    if (in <= 0.f) { //先頭の値を返す
    ...
    else if (in <= 1.f) { //テーブル範囲に収まっている場合、前後の値で線形補間
      ...
      return tables[channel][index] * (1.f - ratio) + tables[channel][index + 1] * ratio;
    }
    // inが1より大きい場合
    // 傾き
    float dv = (tables[channel][table_size - 1] - tables[channel][table_size - 2]) * div_val;
  ...
  if (igs::image::rgba::siz == channels) {
    ...
    for (int ii = 0; ii < pixsize; ++ii, image_array += channels) {
      ...
      /* AlphaのLevel処理した値をrefvの変化量比で加える */
      if (alpha_sw) {
        image_array[alp] = refchk_(image_array[alp], getTableVal(alp,image_array[alp]), refv);
      }
      ...
      /* RGBのLevel処理した値をrefvの変化量比で加える */
      image_array[red] = refchk_(image_array[red], getTableVal(red, image_array[red]), refv);
      image_array[gre] = refchk_(image_array[gre], getTableVal(gre, image_array[gre]), refv);
      image_array[blu] = refchk_(image_array[blu], getTableVal(blu, image_array[blu]), refv);
    }
  }
  else if (igs::image::rgb::siz == channels) {
    using namespace igs::image::rgb;
    ...
    for (int ii = 0; ii < pixsize; ++ii, ++image_array) {
      ...
      image_array[0] = refchk_(image_array[0], getTableVal(0, image_array[0]), refv);
    }
  }
...
void igs::levels::change(
  ...
  if ((std::numeric_limits<unsigned char>::digits == bits) &&
    ...
    assert(std::numeric_limits<float>::digits == ref_bits ||
            0 == ref_bits);
    change_(reinterpret_cast<float*>(image_array), height, width, channels,
      reinterpret_cast<const float*>(ref), ref_mode, r_in_min,
      r_in_max, g_in_min, g_in_max, b_in_min, b_in_max, a_in_min,
      a_in_max, r_gamma, g_gamma, b_gamma, a_gamma, r_out_min, r_out_max,
      g_out_min, g_out_max, b_out_min, b_out_max, a_out_min, a_out_max,
      clamp_sw, alpha_sw, add_blend_sw);
